...
/*
 * blk_start_plug-初始化blk_plug并在task_struct中跟踪它
 * @plug：需要初始化的&struct blk_plug
 *
 * 说明:
 * 如果任务最终在blk_start_plug（）和blk_finish_plug之间阻塞，在task_struct中跟踪blk_plug将有助于自动刷新挂起的I/O.
 * 从性能的角度来看，这很重要，但也能确保我们不会死锁.
 * 例如，如果任务正在阻止内存分配，那么内存回收可能最终想要释放属于该请求的页面，该页面当前位于我们的privete plug.
 */
