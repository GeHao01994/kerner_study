			  ...
			  size_t count, loff_t *ppos)
{
	...
	for (;;) {
		if (!evdev->exist || client->revoked)
			return -ENODEV;
		//如果client是个缓冲区，头部等于尾部，表示是个环形缓冲区
		//表示里面数据是空的，并且文件是已非阻塞的方式打开
		//马上返回一个-EAGAIN，让你再次尝试
			 ...
			 const struct input_device_id *id)
{
	...
	//这里分配了一个evdev结构体，这个evdev结构体中含有input_handle，注意这里和之前的相比没有r
