...
{
//proc_info_list这个是封装的CPU信息的一个链表
	 ...
	 */
	 //这里是查找我们CPU的类型
	 //read_cpuid_id这个应该是读取硬件来得到CPU的ID，往里面追是通过读取CP15协处理器完成的
...
{
	//machine_desc这个是机器码的描述符
	//这里面封装的信息是我们硬件平台封装的所有信息
	...
	//__machine_arch_type这个就是机器码
	//__machine_arch_type这个是在这里查不到的，这个是定义在mach-types.h中定义的
	//但是这个.h里面就包含了一个头文件，就是include/generated/mach-types.h
	//但是这个头文件里面有很多，具体哪个也是跟我们架构相关的。比如我们是SMDKV210架构的话里面就有
	//通过机器码的编号找到对应这个机器码的machine_desc描述符，并且返回这个描述符的指针
	if (!mdesc)
	...
	//这个cmd_line和boot_command_line都是启动参数
