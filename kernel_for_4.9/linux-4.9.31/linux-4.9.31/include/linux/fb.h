...
struct fb_ops {
	...
	//移动视窗，即改变可视屏幕在虚拟屏幕中的位置，若硬件不支持，则不用实现
	/* pan display */
	...
	//绘制光标，若支持硬件检测，可实现方法，否则将由
	//soft_cursor()进行软件检测
	/* Draws cursor */
	int (*fb_cursor) (struct fb_info *info, struct fb_cursor *cursor);
	//对于某些帧缓冲设备来说，必须等待它完成之前的显示处理操作
	//才能继续向帧缓冲中送入显示数据，该方法用于该过程的同步
