...
/* blk_plug允许通过短时间保存I/O片段来构建相关请求的队列.
 * 这允许将顺序请求合并为单个更大的请求.
 * 当请求以批处理的方式从per-task 移动到设备的request_queue时,
 * 这会导致可扩展性的提高，因为request_queue锁的锁争用减少了.
 *
 * 将请求添加到plug list 或尝试合并时，可以不禁用抢占,
 * 因为blk_schedule_flush_list（）只会在任务自身休眠时刷新插入列表.
 * 有关详细信息，请参阅schedule（），其中调用blk_schedule_flush_plug().
 */
/* blk_plug构建了一个缓存碎片IO的请求队列.
 * 用于将顺序请求合并成一个大的请求.
 * 合并后请求批量从per-task链表移动到设备请求队列，
 * 减少了设备请求队列锁竞争，从而提高效率
 * blk_plug的使用很简单
 * 1、设置该线程开启请求合并模式 blk_start_plug
 * 2、关闭线程请求合并blk_finish_plug
*/
struct blk_plug {
	/* 用于缓存请求的队列 */
...
struct block_device_operations {
	...
	/* 这个函数仅适用于支持可移动介质的驱动器,通常需要在驱动中增加一个表示介 质状态是否改变的标志变量,非可移动设备的驱动不需要实现这个方法.
	 * struct gendisk 参数是内核表示一个磁盘的数据结构.
	 */
