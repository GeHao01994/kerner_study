...
struct spi_device {
	...
	//传输时的最大工作频率
	u32			max_speed_hz;
	//设备的片选号，必须小于所属spi_master的num_chopselect
	u8			chip_select;
	//传输时的字节，也就是每个 字数据由多少位构成
	u8			bits_per_word;
	//传输模式，由下列模式位构成
	u16			mode;
	//时钟相位，置位时表示数据在时钟第二个边沿被读取
...
struct spi_master {
	 ...
	 */
	 //控制器对应的总线编号，扳机相关。
	 //例如S3C2440拥有两个SPI控制器，那么总线编号分别是0和1
	s16			bus_num;

	/* chipselects will be integral to many controllers; some others
	 * might use board-specific GPIOs.
	 */
	 //控制器用的片选总数，也就是控制器上可以挂接的设备总数
	u16			num_chipselect;
...
//传输字宽，为0时表示使用SPI设备的默认值
	u8		bits_per_word;
//传输完成到片选禁止之间的(可选项)
	u16		delay_usecs;
//传输时钟，为0表示时，使用SPI设备的默认值
	u32		speed_hz;
	//由SPI_MESSAGE.TRANSFERS连接为一个传输序列
